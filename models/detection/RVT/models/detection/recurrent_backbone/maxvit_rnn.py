...
from models.detection.RVT.models.layers.rnn import DWSConvLSTM2d
from models.detection.RVT.models.layers.s5.s5_model import S5Block
from models.detection.RVT.models.layers.maxvit.maxvit import (
...
class RNNDetector(BaseDetector):
    def __init__(self, mdl_config: DictConfig):
        ...
        for stage_idx, (num_blocks, T_max_chrono_init_stage) in \
            ...
            if hasattr(mdl_config.stage, 'lstm'):
                # lstmが存在する場合、RNNDetectorStageを使用
            ...
            elif hasattr(mdl_config.stage, 's5'):
                
                # s5が存在する場合、RNNSSMDetectorStageを使用
                stage = RNNSSMDetectorStage(
                    dim_in=input_dim,
                    stage_dim=stage_dim,
                    spatial_downsample_factor=spatial_downsample_factor,
                    num_blocks=num_blocks,
                ...
                raise ValueError("mdl_config.backbone.stageに's5'または'lstm'が見つかりません。")
...
class RNNSSMDetectorStage(nn.Module):
    """Operates with NCHW [channel-first] format as input and output."""
