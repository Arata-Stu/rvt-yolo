...
def check_file_pairs(directory):
    ...
    h5_files = [f for f in os.listdir(directory) if f.endswith('_td.h5')]  # ここを修正

    # ベース名（basename）を取得
    npy_base_names = set([f.replace('_bbox.npy', '') for f in npy_files])
    h5_base_names = set([f.replace('_td.h5', '') for f in h5_files])  # ここを修正
...
def process_directory(base_dir, output_base_dir):
    ...
    for split in ['train', 'test', 'val']:
        ...
        for base_name in npy_base_names:
            ...
            h5_file = os.path.join(split_dir, f"{base_name}_td.h5")  # ここを修正
            ...
            shutil.move(h5_file, os.path.join(new_dir, f"{base_name}_td.h5"))  # ここを修正
