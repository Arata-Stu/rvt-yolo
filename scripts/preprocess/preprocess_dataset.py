...
from data.data_utils.representations import MixedDensityEventStack, EventFrame, StackedHistogram, RepresentationBase
...
class H5Writer:
    def __init__(self, outfile: Path, key: str, ev_repr_shape: Tuple, numpy_dtype: np.dtype):
        ...
        self.h5f.create_dataset(key, dtype=np.dtype(self.numpy_dtype).name, shape=chunkshape, chunks=chunkshape,
    ...
    def add_data(self, data: np.ndarray):
        assert data.shape == self.maxshape[1:], f"Shape mismatch: Expected {self.maxshape[1:]}, but got {data.shape}"
...
## modification tensor -> numpy and opencv
def downsample_ev_repr(x: np.ndarray, scale_factor: float):
    assert 0 < scale_factor < 1
    ch, h, w = x.shape
    new_h, new_w = int(h * scale_factor), int(w * scale_factor)
    downsampled = np.zeros((ch, new_h, new_w), dtype=x.dtype)
    for i in range(ch):
        downsampled[i] = cv2.resize(x[i], (new_w, new_h), interpolation=cv2.INTER_NEAREST)
...
def process_sequence(dataset: str,
    ...
    worker_id = current_process().pid  # Worker ID (Process ID)
    ...
    try:
        ...
        # シーケンス名を取得
        sequence_name = in_npy_file.parent.name

        # 出力ファイルの存在チェック
        ev_outfile = out_ev_repr_dir / f"event_representations{'_ds2_nearest' if downsample_by_2 else ''}.h5"
        if ev_outfile.exists():
            print(f"[Worker {worker_id}] Skipping existing sequence: {sequence_name}")
            return
        
        # シーケンスの処理開始ログ
        print(f"[Worker {worker_id}] Loading sequence: {sequence_name}")
        ...
        try:
            ...
            print(f'[Worker {worker_id}] No labels after filtering. Deleting {str(parent_dir)}')
        ...
        print(f"[Worker {worker_id}] Completed sequence: {sequence_name}")

    except Exception as e:
        # エラーが発生した際にシーケンス名とエラーメッセージを表示
        print(f'[Worker {worker_id}] Error processing sequence: {in_npy_file}')
...
class EventFrameFactory(EventRepresentationFactory):
    ...
    def create(self, height: int, width: int) -> EventFrame:
        # EventFrame のインスタンスを生成する
        return EventFrame(img_size = (height, width))
...
if __name__ == '__main__':
    ...
    seq_data_list = list()
    ## modification
    for split in [train_path, val_path, test_path]:  # train, val, testディレクトリを探索
        split_out_dir = target_dir / split.name
        os.makedirs(split_out_dir, exist_ok=True)

        # サブディレクトリ内の.npyファイルをすべて検索
        for npy_file in split.glob('**/*_bbox.npy'):
            h5f_path = npy_file.parent / (
                    npy_file.stem.split('bbox')[0] + f"td{'.dat' if dataset == 'gen1' else ''}.h5")
            assert h5f_path.exists(), f'{h5f_path=}'
            # .h5ファイルが存在するか確認
            
            # ディレクトリ名から出力ディレクトリを生成
            # dir_name = npy_file.relative_to(split).parent / npy_file.stem.split('_bbox')[0]
            dir_name = npy_file.stem.split('_bbox')[0]
            if dir_name in dirs_to_ignore[dataset]:  # dirs_to_ignoreに含まれる場合は無視
                print(f"Ignoring sequence: {dir_name}")  # 確認のためのメッセージ
                continue
